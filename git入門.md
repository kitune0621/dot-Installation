...
(今回は1人のユーザーが一人二役で作業するものとする ) 
Aさんはmywebというファイルを、Bさんはmyweb2というファイルをもっている。
...
※nameはoriginがよく使われる  
$ git remote rm (name) ← remoteを削除する  
$ git push origin master ← 共有リポジトリにいまある内容をプッシュする(この場合、originに対してmasterの内容をプッシュする)  

---

##21 リポジトリの内容を共有してみよう

$ git clone ~/ourweb.git/ myweb2 ← 共有リポジトリの中身をディレクトリに入れる(この場合、myweb2の内容をourweb.gitにする)  
※クローンしたとき、リモートの情報も引き継がれる  
$ git pull ← 共有リポジトリの内容を引っ張りマージをしてくれる  
 
 ---
 
 ##22 共有時のトラブルを解決する
 
 $ git commit -am "(name)" ← addとコミットを同時にする  
 
 他の人が同じ箇所を修正していて、コンフリクトが起きたとき、マージの衝突を解決したように解決出来る。(先にgit pullしろと表示される)
